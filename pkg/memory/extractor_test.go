...
func TestStripCodeFence(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := util.StripCodeFence(tt.input)
			if got != tt.expected {
...
func TestFormatMessagesForExtraction(t *testing.T) {
	messages := []types.Message{
		...
		{Role: "assistant", ToolCalls: []types.ToolCall{{Function: struct {
...
// newExtractorTest creates a MemoryExtractor with a mock server.
func newExtractorTest(t *testing.T, chatResp func(w http.ResponseWriter, r *http.Request), embedResp func(w http.ResponseWriter, r *http.Request)) (*MemoryExtractor, *MemoryStore, *bytes.Buffer, *httptest.Server) {
	t.Helper()
	...
	return extractor, store, output, server
}

// --- ExtractMemories ---

func TestExtractMemoriesNilMemoryStore(t *testing.T) {
	extractor, _, _, server := newExtractorTest(t, nil, nil)
	defer server.Close()

	extractor.Store = nil
	// Should return early without panicking
	extractor.ExtractMemories(context.Background(), []types.Message{
...
func TestExtractMemoriesEmptyMessages(t *testing.T) {
	extractor, store, _, server := newExtractorTest(t, nil, nil)
	defer server.Close()
...
func TestExtractMemoriesNilMessages(t *testing.T) {
	...
	extractor.ExtractMemories(context.Background(), nil)
	if store.Count() != 0 {
		t.Errorf("expected 0 memories, got %d", store.Count())
	}
}
...
func TestExtractMemoriesSuccessfulCreate(t *testing.T) {
	extractor, store, _, server := newExtractorTest(t,
	...
	extractor.ExtractMemories(context.Background(), messages)

	if store.Count() != 1 {
		t.Fatalf("expected 1 memory created, got %d", store.Count())
	}
...
func TestExtractMemoriesInvalidCategory(t *testing.T) {
	...
	if store.Count() != 0 {
		t.Errorf("expected 0 memories for invalid category, got %d", store.Count())
	}
}

func TestExtractMemoriesLLMError(t *testing.T) {
	extractor, store, output, server := newExtractorTest(t,
		func(w http.ResponseWriter, r *http.Request) {
	...
	if !strings.Contains(output.String(), "memory extraction failed") {
		t.Errorf("expected extraction failure message in output, got: %s", output.String())
	}
	if store.Count() != 0 {
		t.Errorf("expected 0 memories on LLM error, got %d", store.Count())
	}
}
...
func TestExtractMemoriesMultipleCandidates(t *testing.T) {
	...
	if store.Count() != 2 {
		t.Fatalf("expected 2 memories created, got %d", store.Count())
	}
}
...
func TestExtractMemoriesWithMerge(t *testing.T) {
	...
	if store.Count() != 1 {
		t.Fatalf("expected 1 memory after merge, got %d", store.Count())
	}

	mem := store.Get("mem_existing_profile")
	if mem == nil {
...
func TestExtractMemoriesOutputMessage(t *testing.T) {
	extractor, _, output, server := newExtractorTest(t,
		func(w http.ResponseWriter, r *http.Request) {
			json.NewEncoder(w).Encode(map[string]interface{}{
				"id":     "chatcmpl-output",
				"object": "chat.completion",
	...
	if !strings.Contains(output.String(), "memories:") {
		t.Errorf("expected status output about memories, got: %s", output.String())
	}
	if !strings.Contains(output.String(), "1 created") {
		t.Errorf("expected '1 created' in output, got: %s", output.String())
	}
}
...
func TestExtractMemoriesEmptyExtraction(t *testing.T) {
	...
	if store.Count() != 0 {
		t.Errorf("expected 0 memories for empty extraction, got %d", store.Count())
	}
	if strings.Contains(output.String(), "memories:") {
		t.Errorf("should not print status when nothing was created, got: %s", output.String())
	}
}
...
func TestLLMExtractMemoriesSuccess(t *testing.T) {
	extractor, _, _, server := newExtractorTest(t,
		func(w http.ResponseWriter, r *http.Request) {
			json.NewEncoder(w).Encode(map[string]interface{}{
	...
	candidates, err := extractor.llmExtractMemories(context.Background(), "User: My name is Alice and I work on Project Alpha")
	if err != nil {
...
func TestLLMExtractMemoriesLLMError(t *testing.T) {
	...
	_, err := extractor.llmExtractMemories(context.Background(), "Some text")
	if err == nil {
...
func TestLLMExtractMemoriesEmptyArray(t *testing.T) {
	...
	candidates, err := extractor.llmExtractMemories(context.Background(), "Some text")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(candidates) != 0 {
...
func TestMergeMemorySuccess(t *testing.T) {
	...
	result := extractor.MergeMemory(context.Background(), existing, candidate, vec)
	if result != 1 {
		t.Errorf("expected merge result 1 (success), got %d", result)
	}

	mem := store.Get("mem_merge_target")
	if mem == nil {
...
func TestMergeMemoryLLMError(t *testing.T) {
	...
	result := extractor.MergeMemory(context.Background(), existing, candidate, nil)
	if result != 0 {
		t.Errorf("expected merge result 0 (failure), got %d", result)
	}

	mem := store.Get("mem_merge_fail")
...
func TestLLMMergeMemoriesSuccess(t *testing.T) {
	...
	merged, err := extractor.llmMergeMemories(context.Background(), existing, candidate)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestLLMMergeMemoriesLLMError(t *testing.T) {
	...
	existing := &Memory{ID: "mem_err", Category: CatProfile, Abstract: "Existing", Overview: "Overview", Content: "Content"}
	candidate := &CandidateMemory{Category: "profile", Abstract: "New", Overview: "New overview", Content: "New content"}

	_, err := extractor.llmMergeMemories(context.Background(), existing, candidate)
	if err == nil {
		t.Error("expected error when LLM fails")
	}
...
func TestLLMMergeMemoriesCodeFence(t *testing.T) {
	...
	existing := &Memory{ID: "mem_fence", Category: CatProfile, Abstract: "Existing", Overview: "Overview", Content: "Content"}
	candidate := &CandidateMemory{Category: "profile", Abstract: "New", Overview: "New overview", Content: "New content"}
