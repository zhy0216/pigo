...
// newDedupTest creates a MemoryDeduplicator with a mock server.
func newDedupTest(t *testing.T, chatResp func(w http.ResponseWriter, r *http.Request)) (*MemoryDeduplicator, *MemoryStore, *httptest.Server) {
	t.Helper()
	...
	return dedup, store, server
}

// --- DeduplicateMemory ---

func TestDeduplicateMemoryNoSimilar(t *testing.T) {
	dedup, _, server := newDedupTest(t, nil)
	defer server.Close()
	...
	result := dedup.DeduplicateMemory(context.Background(), candidate, vec)
...
func TestDeduplicateMemoryProfileAlwaysMerges(t *testing.T) {
	dedup, store, server := newDedupTest(t, nil)
	defer server.Close()
...
func TestDeduplicateMemoryEventsNeverMerge(t *testing.T) {
	dedup, store, server := newDedupTest(t, func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]interface{}{
			"id":     "chatcmpl-events",
			"object": "chat.completion",
			"choices": []map[string]interface{}{
...
func TestLLMDedupDecisionCreate(t *testing.T) {
	dedup, _, server := newDedupTest(t, func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]interface{}{
	...
	candidate := &CandidateMemory{Category: "entities", Abstract: "Project Alpha", Content: "Project details"}
	similar := []SimilarMemory{
		{Memory: &Memory{ID: "mem_sim1", Abstract: "Project Beta", Overview: "Beta overview"}, Score: 0.8},
	}

	result := dedup.llmDedupDecision(context.Background(), candidate, similar)
	if result.Decision != DedupCreate {
		t.Errorf("expected CREATE, got %s", result.Decision)
	}
...
func TestLLMDedupDecisionMerge(t *testing.T) {
	...
	candidate := &CandidateMemory{Category: "entities", Abstract: "Updated Alpha", Content: "Updated details"}
	similar := []SimilarMemory{
...
func TestLLMDedupDecisionLLMError(t *testing.T) {
	...
	candidate := &CandidateMemory{Category: "entities", Abstract: "Something", Content: "Details"}
	similar := []SimilarMemory{
