...
func TestGrepTool_InvalidRegex(t *testing.T) {
	...
	// If rg is available, it may handle invalid regex differently
	// but with native fallback, this should error
	_ = result
}
...
func TestGrepTool_IncludeFilter(t *testing.T) {
	...
	// Native search should filter out .txt
...
func TestGrepTool_LineTruncation(t *testing.T) {
	...
	// Native fallback: line should be truncated at 500 chars
	if result.IsError {
		t.Fatalf("unexpected error: %s", result.ForLLM)
	}
	// The line contains MATCH at position 600, so the truncated display (500 chars) won't include it
	if strings.Contains(result.ForLLM, "MATCH") && len(result.ForLLM) > grepMaxLine+200 {
...
func TestParseRgStream_ContextLines(t *testing.T) {
	...
	// Context lines use double-space separator instead of colon
